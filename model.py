...
from torchvision.transforms import Compose, RandomCrop, RandomResizedCrop, ToPILImage, ToTensor, Lambda, \
    RandomHorizontalFlip, RandomRotation, RandomAffine, RandomPerspective, Grayscale, Normalize, Resize, CenterCrop, \
    Pad, Normalize
...
def _y_pred_to_grade(y_pred):
    return [int(round(i * 10)) for i in y_pred]
...
def predict_image(model, img):
    ...
    model.to(device)
    y_pred = np.zeros(criterion_number)
...
def test(model, model_name_, val, predict_folder):
    ...
    if not predict_folder:
        for i in range(len(y_pred)):
            print(f'{test_dataset.get_img_name(i)}: {_y_pred_to_grade(y_pred[i])} {_y_pred_to_grade(y_true[i])}')
        ...
        for i in range(len(y_pred)):
            print(f'{test_dataset.get_img_name(i)}: {_y_pred_to_grade(y_pred[i])}')
...
if __name__ == '__main__':
    ...
    if namespace.unpack:
        img = read_img_from_base64(test_64)
